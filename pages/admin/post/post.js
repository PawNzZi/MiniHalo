...
Page({
  ...
  data: {
    optionArray: [
      { text: '已发布', value: 0 ,status:'PUBLISHED'},
      { text: '有密码', value: 1 ,status:'INTIMATE'},
      { text: '草稿箱', value: 2 ,status:'DRAFT'},
      { text: '回收站', value: 3 ,status:'RECYCLE'},
    ],
    articleList:[],
    isEmpty:false,
    showShare: false,
  ...
  onSearch:function(e){
    ...
    this.setData({keyword:keyword,articleList:articleList,status:status});
  ...
  dropdownChange:function(e){
    ...
    this.setData({articleList:articleList,page:0,status:object.status});
    keyword = App.checkTextIsEmpty(keyword);
    this.postSearch(keyword,0,object.status);
  },
  touchItem:function(e){
    var item = e.detail.item;
    this.setData({item:item,showShare:true});
  },
  onClose: function () {
    this.setData({
      showShare: false
